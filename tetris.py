...
class AI(Player):
    ...
    async def stop_game(self):
        ...
        if self.prog.stdin:
            self.prog.stdin.close()
            try:
                self.prog.stdin._transport.__del__() # 3.10 fix
            except:
                pass
...
def is_valid_placement(
    board: list[list[int]], piece_name: str, x: int, rotation: int
) -> bool:
...
async def game(
    players: list[Human | AI], debug: bool, **kwargs
    ...
    async def play_solo_game(player: Human | AI):
        ...
        while player.alive:
            ...
            await Player.print(
                f"Score: {player.score} | Pieces placed: {player.pieces_placed}"
            )
            ...
            if not user_input:
                ...
                lines_cleared = place_piece(
                    player.board, player.current_piece_name, x, rotation
                )
